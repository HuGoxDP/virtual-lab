...
> **`id` у каталозі та `id` у маніфесті можуть відрізнятись — і зараз не відрізняються.**
> Рушій їх не звіряє: `loadScenarioFromBuffer` отримує лише байти. Тому розбіжність
> показується як попередження, а не помилка, а `manifest_id` зберігається в БД і видно
> в адмінці. Старі архіви використовували зворотно-доменні ідентифікатори
> (`template.benchscene1.primitives`) при слагах у каталозі; ScenarioCreator тепер видає
> слаг, і всі 13 рядків збігаються. Робити з попередження помилку на цій підставі **не
> варто** — це відкине будь-який старіший архів, а рантайму збіг не потрібен.
...
виграшу. Подробиці й потрібний замір — [`../../.claude-shared/PLAN.md`](../../.claude-shared/PLAN.md).
